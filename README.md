...
- Audit logging: every login and every real action (password reset, ticket, software access, incident report) is recorded with timestamp, user, and details in a local log; viewable and filterable (by user and event type) in-app, with a separate collapsible analytics view (events by type, activity over time, most active users)
...
- FAQ auto-generation: analyzes logged knowledge questions to identify recurring themes, then generates a grounded FAQ list using the actual knowledge base (not guessed answers), citing sources and honestly flagging anything not covered
